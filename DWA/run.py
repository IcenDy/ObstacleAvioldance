...
def main():
    # init
    ''' robot '''
    x_r = np.array([0, 0, utils.deg2rad(18.0), 0, 0], dtype=float).reshape(5 ,1)
    limits = [1.0, utils.deg2rad(20), 0.2, utils.deg2rad(50), 0.01, utils.deg2rad(1)]
    params_rob = [0.0, 0.2]
    params_dwa = [0.045, 0.1, 0.1]
    dt = 0.1
    robot = agents.Robot(x_r, limits, params_rob, params_dwa, dt)
    ''' lidar '''
    x_l = np.array([0, 0, utils.deg2rad(18.0)],dtype=float).reshape(3, 1)
    ...
    ob_0 = ob_s
    obstacleR = 0.5 * np.ones(4, dtype=float)  # radius of obstacles
    ob_0 = np.c_[ob_0, obstacleR]  # 7*3 array
    env.obstacles.append(ob_0)

    ob_e0 = np.c_[np.linspace(-1, 11, 13, True, dtype=float), -2 * np.ones(13, dtype=float)]
    edge_0 = np.c_[ob_e0, 0.5 * np.ones(13, dtype=float)]  # 15*3 array: bottom
    env.obstacles.append(edge_0)

    ob_e1 = np.c_[12 * np.ones(15, dtype=float), np.linspace(-2, 12, 15, True, dtype=float)]
    edge_1 = np.c_[ob_e1, 0.5 * np.ones(15, dtype=float)]  # 15*3 array: right
    env.obstacles.append(edge_1)

    ob_e2 = np.c_[np.linspace(0, 12, 13, True, dtype=float), 13 * np.ones(13, dtype=float)]
    edge_2 = np.c_[ob_e2, 0.5 * np.ones(13, dtype=float)]  # 15*3 array: top
    ...
    while (steps < 5e3):
        ...
        robot.dwa_decision(env.goals[:, -1], env.obstacles)
